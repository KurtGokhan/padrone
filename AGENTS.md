...
**Extension system**: Build-time composition via `.extend(extension)`. A `PadroneExtension` is a function that receives the builder and returns a modified builder, enabling reusable command/config bundles. Unlike interceptors (which hook into runtime phases), extensions operate at definition time to compose commands, arguments, and configuration. Built-in features (help, version, repl, color, config, interactive, signal handling, auto-output) are included by default via `createPadrone()`. User-facing builtins can be individually disabled via `{ builtins: { help: false } }`. Framework extensions (signal, auto-output, config, interactive) are always on by default. Advanced features (completion, man, mcp, serve, update-check) are opt-in extensions.
